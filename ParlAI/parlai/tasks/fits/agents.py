...
def format_to_woi_consistent(docs, woi_format_gold_docs):
    # organize into wizard of internet format
    retrieved_docs = []
...
class FitsSatisfactionTeacher(FitsBaseTeacher):
    ...
    def setup_data(self, data_path):
        ...
        random.shuffle(data)
        # output_file = open("parlai_test_unseen.json", "w")
        for turn in data:
            message = Message(turn)
            # print(json.dumps(turn), file=output_file)
            yield message, True
        # output_file.flush()
        # exit(0)
...
class QueryTeacher(FitsBaseTeacher):
    ...
    def __init__(self, opt, shared=None):
        # By default, we only use human_gold
...
"""
New added teachers for System-level NL Feedback
"""


class MyQueryTeacher(QueryTeacher):
    """ Use my collected data to train the fits query generator"""
    ...
    def add_cmdline_args(
        ...
        agent.add_argument(
            "--my-train-data",
            type=str,
            help="My organized training data"
        )
        agent.add_argument(
            "--my-valid-data",
            type=str,
            help="My organized valid data"
        )
        agent.add_argument(
            "--my-test-data",
            type=str,
            help="My organized test data"
        )
        agent.add_argument(
            "--my-test-unseen-data",
            type=str,
            help="My organized test unseen data"
        )
        return parser
    ...
    def __init__(self, opt, shared=None):
        # By default, we only use human_gold
        self.query_source = opt.get('query_source', None)
        self.my_train_path = opt.get("my_train_data", None)
        self.my_valid_path = opt.get("my_valid_data", None)
        self.my_test_path = opt.get("my_test_data", None)
        self.my_test_unseen_path = opt.get("my_test_unseen_data", None)
        super().__init__(opt, shared)
        self.id = f'fits_query_{opt["mutators"]}'
    ...
    def setup_data(self, data_path):
        if self.datatype.startswith('train'):
            # Load my own data
            data = []
            with open(self.my_train_path, "r") as f:
...
class MyFitsBaseTeacher(FitsBaseTeacher):
    @classmethod
    def add_cmdline_args(
            cls, parser: ParlaiParser, partial_opt: Optional[Opt] = None
    ) -> ParlaiParser:
        ...
        agent.add_argument(
            "--my-valid-data",
            type=str,
            help="My organized validation data, containing retrieved docs"
        )
        agent.add_argument(
            "--my-test-data",
            type=str,
            help="My organized test data, containing retrieved docs"
        )
        agent.add_argument(
            "--my-test-unseen-data",
            type=str,
            help="My organized test unseen data, containing retrieved docs"
        )
        return parser
    ...
    def __init__(self, opt, shared=None):
        ...
        self.unseen_task = opt.get('unseen_task', False)
        self.my_train_data = opt.get("my_train_data", None)
        self.my_valid_data = opt.get("my_valid_data", None)
        self.my_test_data = opt.get("my_test_data", None)
        self.my_test_unseen_data = opt.get("my_test_unseen_data", None)
        super().__init__(opt, shared)
        self.id = f'fits_{opt["mutators"]}_{opt["fits_task_version"]}_seen{opt["unseen_task"]}'
    ...
    def setup_data(self, data_path):
        if self.datatype.startswith("train"):
            ...
            data = []
            ignored_count = 0
            with open(self.my_train_data, "r") as f:
                for line in f.readlines():
                    line = line.strip()
                    try:
                        line = json.loads(line)
                    ...
                    labels = [line["response"]]
                    data.append({
                        "text": full_text_list[-1],
                        "labels": labels,
                        "full_text_list": full_text_list,
                        "__retrieved-docs__": line["__retrieved-docs__"],
                        "__selected-sentences__": line["__selected-sentences__"],
                        "__retrieved-docs-titles__": line["__retrieved-docs-titles__"]
                    })
            ...
            if self.datatype.startswith("valid"):
                print("My valid data")
                my_data_path = self.my_valid_data
            elif self.datatype.startswith("test") and not self.unseen_task:
                print("My test data")
                my_data_path = self.my_test_data
            elif self.datatype.startswith("test") and self.unseen_task:
                print("My test unseen data")
                my_data_path = self.my_test_unseen_data
            else:
                raise NotImplementedError
            if my_data_path is None:
                exit(0)
            with open(my_data_path, "r") as f:
                for line in f.readlines():
                    line = line.strip()
                    line = json.loads(line)
                    start_of_episode = line["start_of_episode"]
                    yield Message(line), start_of_episode
