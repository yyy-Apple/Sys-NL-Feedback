...
class History(object):
    ...
    def get_history_vec_from_full_text_list(self, full_text_list):
        if len(full_text_list) == 0:
            return None
        history = []
        for text in full_text_list[:-1]:
            history += [self.parse(text)]
            history += [self.delimiter_tok]
        history += [self.parse(full_text_list[-1])]
        if self.temp_history is not None:
    ...
    def get_history_vec(self):
        ...
        # for fits, the self.temp_history is None
        if self.temp_history is not None:
            history.extend([self.parse(self.temp_history)])
        if self._global_end_token is not None:
            history += [[self._global_end_token]]
    ...
    def _set_text_vec(self, obs, history, truncate):
        ...
        if 'text_vec' not in obs:
            ...
            if history_string is None:
                return obs
            if "full_text_list" not in obs:
                obs['full_text'] = history_string
                if history_string:
                    obs['text_vec'] = history.get_history_vec()
                    obs['full_text_vec'] = history.get_history_vec()
            else:
                # We always go to this branch
                obs["full_text"] = history.delimiter.join(obs["full_text_list"])
                obs["text_vec"] = history.get_history_vec_from_full_text_list(obs["full_text_list"])
                obs["full_text_vec"] = history.get_history_vec_from_full_text_list(obs["full_text_list"])
